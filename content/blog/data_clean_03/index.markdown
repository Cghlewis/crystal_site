...
4. I am reviewing my data as soon as I import it. Using the `glimpse()` function provides me summary information about my data such as the number of rows and columns, my variable names, and my variable formats.
    - I am happy to see that the number of variables in my data match what I see in my data dictionary (7 primary variables) and the formats are all as expected.
    ...
    - I can also see that my variable format for `math2` is not as expected. The variable is character and I expected it to be numeric. There's something funky going on in this variable. Note that in a real (non-fictitious scenario), this mistake **should** have been caught and fixed in our double-entry checking process above. But for demonstration purposes, I left this mistake in. :)
